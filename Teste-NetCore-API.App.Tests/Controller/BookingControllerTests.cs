...
namespace Teste_NetCore_API.App.Tests.Controller
{
    public class BookingControllerTests
    {
        ...
        public void GetAll_ReturnsOkResult_WithListOfBookings()
        {
            ...
            mockService.Setup(service => service.GetAll()).Returns(expectedBookings);
        ...
        [Fact]
        public void GetById_WithValidId_ReturnsOkResult_WithBooking()
        {
            // Arrange
            int id = 1;
            ...
            var mockService = new Mock<IBookingDomainService>();
            mockService.Setup(service => service.GetById(id)).Returns(expectedBooking);
            var controller = new BookingController(mockService.Object);
        ...
        public void GetById_WithInvalidId_ReturnsNotFoundResult()
        {
            ...
            int id = 1;
            var mockService = new Mock<IBookingDomainService>();
            mockService.Setup(service => service.GetById(id)).Returns((Booking)null);
            var controller = new BookingController(mockService.Object);

            // Act
            var result = controller.GetById(id) as NotFoundObjectResult;
